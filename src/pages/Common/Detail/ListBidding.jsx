...
const ListBidding = ({ listBidding, currentUser, createdBy, handlePageChange, handleOpenBidding, detail, page, totalPage }) => {
            ...
            <Tooltip title="Kỹ năng ">
                <Box display='flex' gap={2}>
                    {listSkill?.map((item, index) => (
                        <Typography key={index} fontSize='15px' fontWeight='bold'> {item} </Typography>
                    ))}
                </Box>
            </Tooltip>)
                        ...
                        <Paper sx={{ bgcolor: '#F8F8FF', borderRadius: '5px', mt: 1 }}>
                            <Box p={3}>
                                <Box display='flex'>
                                        ...
                                        <Box ml={2}>
                                            ...
                                            <Box sx={{ display: 'flex', gap: 2 }}>
                                                ...
                                                <Tooltip title="Tổng số đánh giá">
                                                    <Typography><HowToVoteIcon /> {item?.appUser2?.totalRate}</Typography>
                                                </Tooltip>
                                                <Tooltip title="Địa chỉ">
                                                    {(item?.appUser2?.city != null || item?.appUser2?.country != null) && (<>
                                                        <Typography><FmdGoodIcon color='primary' /> {item?.appUser2?.city} {item?.appUser2?.country}  </Typography>
                                                    </>)}
                                                </Tooltip>
                                            </Box>
                                    ...
                                    <Box ml='auto'>
                                        <Typography> Ngân sách : {item?.budget}$ </Typography>
                                        <Typography> Thời gian :  {item?.duration} ngày </Typography>
                                    </Box>
                                </Box>
                                ...
                                {currentUser != null && currentUser?.role === ROLES.RECRUITER && currentUser?.userId === createdBy && detail.projectStatus.id !== 3 && (
                                    <>
                                        <Box display='flex' mt={1} ml='auto'>
                                            <Button variant='contained' onClick={(e) => handleOpenBidding(item?.id)}>Chấp nhận</Button>
                                        </Box>
                                    </>
                                )}
                                <Box display='flex' justifyContent='space-between' alignItems='center' mt={1}>

                                    <Typography fontSize='12px' mt={2}>Thời gian cập nhật : {formatDateTime(item?.updatedDate)}</Typography>
                                    <Box display='flex' alignItems='center'>
