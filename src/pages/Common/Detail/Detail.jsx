...
const Detail = () => {
    ...
    useEffect(() => {
        const getAllBidding = async () => {
            if (value === '2') {
                setLoading(true);
                try {
                    let res = await biddingApi.GetBiddingListByProjectId(projectId, page, 5);
    ...
    const handleSubmit = async (e) => {
        ...
        setLoading(true);
        let isError = false;
        if (budget === 0 || budget >= 2000000000 || budget === "" || budget === "0") {
            seterrorBudget(true);
            isError = true
        ...
        if (duration <= 0 || duration === "" || duration === "0" || duration >= 100) {
    ...
    const handleSubmitUpdate = async (e) => {
        e.preventDefault();
        let isError = false;
        if (budgetUpdate === 0 || budgetUpdate >= 2000000000 || budgetUpdate === "" || budgetUpdate === "0") {
        ...
        if (durationUpdate <= 0 || durationUpdate === "" || durationUpdate === "0" || durationUpdate >= 100) {
    ...
    return (
        <>
            ...
            <Modal
                ...
                <Box sx={style}>
                    <Box p={2}>
                        <Typography fontSize='18px' fontWeight='bold'>Đặt giá đấu thầu cho dự án </Typography>
                    </Box>
                    <Divider />
                    <Box pt={3} pl={3}>
                        <Typography fontSize='15px'>Bạn sẽ có thể chỉnh sửa giá thầu của mình cho đến khi dự án được trao cho ai đó.</Typography>
                    </Box>
                    <Box m={5}>
                    ...
                    <Box display="flex" justifyContent="space-between" p={2}>
                        <Button variant='contained'
                            disabled={loading}
                            onClick={handleSubmit}>Đấu thầu</Button>
                    </Box>
                </Box>
            </Modal>
            {/* Delete ============================ */}
            ...
            >
                <Box sx={styleDelete}>
                    ...
                    <Button variant="outlined" color='error' onClick={(e) => handleConfirmDelete()} sx={{ mr: 2 }}>
                        Chắc chắn
                    </Button>
            ...
            >
                <Box sx={styleDelete}>
                    ...
                    <Button variant="outlined" color='error' onClick={(e) => handleAccept()} sx={{ mr: 2 }}>
                        Chắc chắn
                    </Button>
            ...
            <Modal
                ...
                <Box sx={style}>
                    ...
                    <Box display="flex" justifyContent="space-between" p={2}>
                        <Button variant='contained'
                            disabled={loading}
                            onClick={(e) => handleSubmitUpdate(e)}>Chỉnh sửa</Button>
                    </Box>
                </Box>
            </Modal>
        </>
