import { Box, Button, IconButton, Modal, Switch, Tooltip, Typography } from '@mui/material'
...
const ViewBlog = () => {
  ...
  const updateItem = (blogId) => {
    setListBlog((prev) =>
      prev.map((item) =>
        item.blogId === blogId ? { ...item, isPublished: !item.isPublished } : item
      )
    );
  };
  ...
  const removeItem = (blogId) => {
    setListBlog((prev) =>
      prev.filter((item) => item.blogId !== blogId)
    );
  }
    ...
    <Box className="p-4" sx={{ width: '100%', overflow: 'auto' }}>
      ...
      >
        <Box sx={style}>
          ...
          <Button variant="outlined" color='error' onClick={(e) => handleConfirm()} sx={{ mr: 2 }}>
            Chắc chắn
          </Button>
