...
const UserList = ({
  ...
  const columns = [
    ...
    { field: 'email', headerName: 'Email', width: 200, flex: 1 },
    {
      ...
      renderCell: (params) => (
        <Box display="flex" alignItems="center" width="100%" sx={{ mt: 1.5 }}>
          <Typography>
            {params.value ? params.value : t('no_information', { ns: 'admin' })}
          </Typography>
        </Box>
      ),
    },
    ...
    },
    {
      ...
      renderCell: (params) => (
        <Box display="flex" alignItems="center" width="100%" sx={{ mt: 1.5 }}>
          <Typography>
            {params.row.isCompany ? 'Recruiter' : 'Freelancer'}
          </Typography>
        </Box>
      ),
    },
        ...
        <Box>
          {params.row.isLock ? (
            <Tooltip title="Unlock user">
              <IconButton
                onClick={() => onOpenModal(params.row, 'unlock')}
                color="success"
              >
                <LockOpenIcon />
              </IconButton>
            ...
            <Tooltip title="Lock user">
              <IconButton
                onClick={() => onOpenModal(params.row, 'lock')}
                color="error"
              >
                <LockPersonIcon />
              </IconButton>
  ...
  const handleBulkLockUser = async () => {
    ...
    const { lockedUsers, activeUsers } = usersData.reduce(
      (acc, user) => {
        if (user.isLock) {
        ...
        InputProps={{
          startAdornment: (
            <InputAdornment position="start">
              ...
              >
                <MenuItem onClick={() => handleSearchBySelect('Name')}>
                  Name
                </MenuItem>
                <MenuItem onClick={() => handleSearchBySelect('Email')}>
                  Email
                </MenuItem>
                <MenuItem onClick={() => handleSearchBySelect('Phone Number')}>
                  Phone Number
                </MenuItem>
              </Menu>
            </InputAdornment>
          ...
          endAdornment: (
            <InputAdornment position="end">
              ...
              >
                ...
                <MenuItem onClick={() => handleRoleSelect('Freelancer')}>
                  Freelancer
                </MenuItem>
                <MenuItem onClick={() => handleRoleSelect('Recruiter')}>
                  Recruiter
                </MenuItem>
            ...
            pagination: {
              ...
              labelDisplayedRows: ({ from, to }) => {
                ...
                return `${from.toLocaleString('en')}-${to.toLocaleString(
                  'en'
                )} ${t('of_page')} ${totalPages.toLocaleString('en')} ${t(
                  'page'
                )}`;
              },
            },
      ...
      >
        ...
        <DialogContent>
          <DialogContentText id="alert-dialog-description">
            Chúng tôi không khuyến khích hành động khóa/mở khóa toàn bộ người
            dùng đã chọn. Điều này có thể dẫn đến các sai lầm và khó khăn trong
            việc chỉnh sửa về sau.
          </DialogContentText>
